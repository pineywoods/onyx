        ...
        try:
            ...
            # code files (.py, .js, .rs, etc.) via its is_text_file() fallback.
                ...
                if not text_content.strip():
                    ...
                    # `count` is only populated when image extraction is
                    # enabled, so the docling branch must not depend on it.
                    if image_bearing_ext and (count > 0 or get_docling_url()):
                        results.acceptable.append(upload)
